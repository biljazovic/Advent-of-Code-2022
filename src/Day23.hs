...
simulate (mat, order) = (mat', drop 1 order)
  where
    ...
    proposeInDir pos dir_ind = 
      if all space $ map (+pos) [dirs8 !! ind | ind <- [dir_ind-1..dir_ind+1]]
    ...
    mat' = Set.fromList $ concat $ map (\(pos', poss) -> if length poss == 1 then [pos'] else poss) (Map.assocs proposed)
...
solve matc = matF
  where
    matF = (length $ (iterateUntil (\((m1,_),(m2,_)) -> m1==m2) simulate (mat, testOrder))) - 1
